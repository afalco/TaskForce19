...
En la estimación es están utilizando los siguientes puntos de control:
...
mean_error <- 10^{mean(floor(log10(x)),na.rm = TRUE)}
...
```
El error relativo promedio en el ajuste es de `r mean_error`
...
A día 6/04/2020 hay un total de 7330 casos registrados en la Comunidad Valenciana, de los cuales 5763 permanecen infectados en la actualidad. Partiendo desde el día 1/03/2020 se han realizado diferentes escenarios de simulación dependiendo de la medida de control implantada en una fecha determinada:

```{r echo=FALSE}
kable(escenarios[,c("escenario", "cases")], digits=0, col.names = c("Escenario", "Casos reportados"))
```
