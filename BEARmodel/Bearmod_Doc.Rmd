...
# El modelo empleado en el código


La notación que emplearemos es $1 \le k \le m$ denota el número de nodos en el dividimos la población. El modelo
supone que en $t=0$ se conocen los valores de la población del nodo $k$ que denotaremos por $N_k.$ El modelo
describe la evolución de $E_k(\ell\Delta t),$ $I_k(\ell\Delta t)$ y $R_k(\ell\Delta t)$ el número de indivíduos expuestos,
infectados y recuperados. Los individuos susceptibles se calculan empleando
$$S_k(\ell \Delta t)=N_k-E_k(\ell \Delta t)-I_k(\ell\Delta t)- R_k(\ell\Delta t)$$
donde $\Delta t$ es un día y $0\le \ell \le M$ son los días que deseamos simular.

Los datos iniciales son $(E_k(0),I_k(0),R_k(0),N_k)$ para $1 \le k \le m.$ Tambien conocemos los datos de movilidad que se modelan empleando una matriz de Markov
$$
P(\ell \Delta t) = (p_{ij}(\ell \Delta t)) \in [0,1]^{m\times m},
$$  
donde $p_{ij}(\ell \Delta t)$ representa la probabilidad que un individuo del nodo $i$ se desplaze al nodo $j$
entre $\ell-1$ y $\ell$ para $1 \le \ell \le M.$ Esta matriz tiene que cumplir que
$$
\sum_{j=1}^m p_{ij}(\ell \Delta t) = 1 \text{ para todo } 1 \le i \le m \text{ y } 1 \le \ell \le M.
$$
Tenemos entonces que los individuos del nodo $i$ que se desplazan al nodo $j$ es entonces
$$
p_{ij}(\ell \Delta t) N_i
$$
el movimiento de población entre $\ell-1$ y $\ell$ en un nodo genérico $i$ viene determinado por
$$
\sum_{j=1}^m p_{ji}(\ell \Delta t) N_j - \sum_{j=1}^m p_{ij}(\ell \Delta t) N_i.
$$
La dinámica del modelo viene determinada por la function `runSim` y en particular por la estructura
```R
...
```
La primera función repite el procedimiento siguiente para construir la dínamica: Considera  
$$
(E_k(0),I_k(0),R_k(0),N_k)
$$
y calcula 
$$
(E_k(\Delta t),I_k(\Delta t),R_k(\Delta t)
$$
del siguiente modo para $\Delta t =$ `current_day = 1`
$$
R_k(\Delta t) = R_k(0) + \sum_{i=1}^{I_k(0)} X_i \text{ con } X_i \sim \mathcal{B}(\lambda_1)
$$
aquí $X_1,\ldots,X_{I_k(0)}$ es una muestra aleatoria simple de una variable aleatoria 
Bernouilli de parámetro $\lambda_1 =$ `recrate`.
$$
E_k(\Delta t) = E_k(0) - \sum_{i=1}^{E_k(0)}Y_i + \left(
1 - \frac{E_k(0)+I_k(0)+R_k(0)}{N_k}
\right) \sum_{i=1}^{I_k(0)+\gamma E_k(0)} Z_i
$$
donde $Y_1,\ldots,Y_{E_k(0)}$ es una muestra aleatoria simple de una variable aleatoria 
Bernouilli de parámetro $\lambda_2 =$ `exp_to_infrate`
$Z_1,\ldots,Z_{I_k(0)+\gamma E_k(0)}$ siendo $\gamma =$ `exposed_pop_inf_prop` es una muestra aleatoria simple de una variable aleatoria Poisson de parámetro $\lambda_3 =$ `exposerate`. Finalmente,
$$
I_k(\Delta t) = I_k(0) + \sum_{i=1}^{E_k(0)}Y_i -  \sum_{i=1}^{I_k(0)} X_i + \sum_{j=1}^m p_{jk}(\Delta t) N_j - \sum_{j=1}^m p_{kj}(\Delta t) N_k.
$$
Almacenamos $(E_k(\Delta t),I_k(\Delta t),R_k(\Delta t))$ y tomamos $(E_k(0),I_k(0),R_k(0)) \leftarrow (E_k(\Delta t),I_k(\Delta t),R_k(\Delta t))$ y repetimos el proceso anterior almacenando el resultado en
$(E_k(2\Delta t),I_k(2\Delta t),R_k(2\Delta t))$ y así seguimos hasta conseguir
$(E_k(M\Delta t),I_k(M\Delta t),R_k(M\Delta t)).$
...
exposedTimeStep = function(HPop, exposerate_df, current_day, exposed_pop_inf_prop){
  ...
  for (i in 1:length(HPop$nInf)){
    ...
    #HPop$nExposedToday[i]= sum(rbinom(infectious_pop,1,exposerate)) 
    # * (1 - ( (HPop$nInf[i] + HPop$nExp[i]) / HPop$nTotal[i]))
    RandomS = sum(rpois(infectious_pop,exposerate))
    Exposed_fraction = 1-(HPop$nInf[i] + HPop$nExp[i] + HPop$nRec[i]) / HPop$nTotal[i]
    HPop$nExposedToday[i]= RandomS * Exposed_fraction
...
movementTimeStep = function(HPop, mobmat,day,control_df,prob_move_per_TS){
  ...
  #     - movement_matrix(i,j) representa la probabilidad p de que un invididuo 
  #       de la region i se mueve a j
  #     - Se toma el número de infectados de la region i como numero de ensayos 
  #       de la distribucion binomial
  #     - Se genera un valor aleatorio a partir de dicha distribucion para cada 
  #       entrada, que representa los infectados que se han movido de i a j 
  #       (incluyendo los que se quedan en la misma región)
  ...
  # Se normaliza cada fila de la matriz de movimientos y se multiplican por el 
  # número de infectados en i para asegurarnos que la suma de cada fila sea igual 
  # al número de infectados en cada región
  ...
  # A la población infectada de cada patch, se le restan los que salen y 
  # se suman los que entran
  HPop$nInf = HPop$nInf - floor(rowSums(moved_matrix)) + ceiling(colSums(moved_matrix))
  #quick fix

  # Evita que el numero de infectados de una region pueda ser superior al numero 
  # total de habitantes
  for (i in 1:length(HPop$nInf)){
...
#relative_movement is the proportion of original movement out/in 
#that we want to keep -- ie. .1 = 10% of original movement rate
#######################################################
